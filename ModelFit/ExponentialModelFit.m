% This script file demonstrates the fitting of an exponential model on the
% monthly CO2 dataset.
...
% Set up the exponential fit model parameters.
model_name = 'Exponential Model ';
coefs = {'B','D','A'};
parameters_number = length(coefs);
% Set up the actual functional form of the fitting model.
functional_form = 'D*exp(-A*x)+B';
...
% Evaluate the exponential model fit on the test data.
